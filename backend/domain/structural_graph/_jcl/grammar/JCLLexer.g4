...
{
System.out.println(getLine() + ":" + self.column + " / " + self.text + "/");
}
{
...
tokens {
NAME , ALNUMNAT , DELIMITER_STATEMENT , COMMENT_FLAG , CNTL , COMMAND , DD , ELSE , ENDCNTL , ENDIF , EXEC , IF , INCLUDE , JCLLIB , JOB , NOTIFY , OUTPUT , PEND , PROC , SCHEDULE , SET , XMIT, EQUAL , ACCODE , AMP , ASTERISK , AVGREC , BLKSIZE ,  BLKSZLIM , BUFNO , BURST , CCSID , CHARS , CHKPT , COPIES , DATA , DATACLAS , DCB , DDNAME , DEST , DIAGNS , DISP , DLM , DSID , DSKEYLBL , DSN , DSNAME , DSNTYPE , DUMMY , DYNAM , EATTR , EXPDT , EXPORT , FCB , FILEDATA , FLASH , FREE , FREEVOL , GDGORDER , HOLD , KEYLABL1 , KEYLABL2 , KEYENCD1 , KEYENCD2 , KEYLEN , KEYOFF , LABEL , LGSTREAM , LIKE , LRECL , MAXGENS , MGMTCLAS , MODE, MODIFY , NULLOVRD , OUTLIM , OUTPUT , PATH , PATHDISP , PATHMODE , PATHOPTS , PROTECT , RECFM , RECORG , REFDD , RETPD , RLS , ROACCESS , SECMODEL , SEGMENT , SPACE , SPIN , STORCLAS , SUBSYS , SYMBOLS , SYMLIST , SYSOUT , TERM , UCS , UNIT , VOL , VOLUME , COMMA , ABEND , ABENDCC , FAIL , NOT_SYMBOL , TRUE , FALSE , RC , RUN , NUM_LIT , LPAREN , RPAREN , BFALN , BFTEK , BUFIN , BUFL , BUFMAX , BUFOFF , BUFOUT , BUFRQ , BUFSIZE , CODE , CPRI , CYLOFL , DEN , DSORG , EROPT , FUNC , GNCP , HIARCHY , INTVL , IPLTXID , LIMCT , NCP , NTM , OPTCD , PCI , PRTSP , RESERVE , RKP , SOWA , STACK , THRESH , TRTCH , ADDRSPC , BYTES , CARDS , CCSID , CLASS , COND , DSENQSHR , EMAIL , GDGBIAS , GROUP , JESLOG , JOBRC , LINES , MEMLIMIT , MSGCLASS , MSGLEVEL , NOTIFY , PAGES , PASSWORD , PERFORM , PRTY , RD , REGION , REGIONX , RESTART , ROLL , SECLABEL , SYSAFF , SCHENV , SYSTEM , TIME , TYPRUN , UJOBCORR , USER , COMMENT_TEXT , DOT , QUOTED_STRING_PROGRAMMER_NAME , SUBCHARS , SEP , JOB_ACCT_MODE_UNQUOTED_STRING
}
...
fragment ANYCHAR
	...
	;
fragment NAME
	: (NATL | ALPHA) (ALPHA | NATL | NUM)* {len(self.text) < 9}?
	;
NUM_LIT_DFLT
	: NUM+
	->type(NUM_LIT)
...
DATA_MODE_TERMINATOR1
	: SLASH SLASH ASTERISK {"//" in self.dlmVals and self.column == 3}? 
	{
self.dlmVals = []
self._modeStack.clear()
self.myMode = 0
...
DATA_MODE_TERMINATOR2
	: SLASH SLASH {"//" in self.dlmVals and self.column == 2}? 
	{
self.dlmVals = []
self._modeStack.clear()
self.myMode = 0
...
DATA_MODE_TERMINATOR3
	: SLASH ASTERISK {"/*" in self.dlmVals and self.column == 2}? 
	{
self.dlmVals = []
self._modeStack.clear()
self.myMode = 0
...
DATA_MODE_TERMINATORX
	: ANYCHAR+ {self.text in self.dlmVals}?
	{
self.dlmVals = []
self._modeStack.clear()
self.myMode = 0
